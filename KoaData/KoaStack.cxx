...
#include "KoaMCTrack.h"                // for KoaMCTrack
...
// -----   Default constructor   -------------------------------------------
KoaStack::KoaStack(Int_t size)
  : FairGenericStack(),
    fStack(),
    fParticles(new TClonesArray("TParticle", size)),
    fTracks(new TClonesArray("KoaMCTrack", size)),
    fStoreMap(),
    fStoreIter(),
    fIndexMap(),
    fIndexIter(),
...
TParticle* KoaStack::PopNextTrack(Int_t& iTrack)
{
...
TParticle* KoaStack::PopPrimaryForTracking(Int_t iPrim)
{
  ...
  if (iPrim < 0 || iPrim >= fNPrimaries) {
    fLogger->Fatal(MESSAGE_ORIGIN, "KoaStack: Primary index out of range! %i ", iPrim );
  ...
  if ( ! (part->GetMother(0) < 0) ) {
    fLogger->Fatal(MESSAGE_ORIGIN, "KoaStack:: Not a primary track! %i ",iPrim);
...
TParticle* KoaStack::GetCurrentTrack() const
{
  TParticle* currentPart = GetParticle(fCurrentTrack);
  if ( ! currentPart) {
    fLogger->Warning(MESSAGE_ORIGIN,"KoaStack: Current track not found in stack!");
...
void KoaStack::AddParticle(TParticle* oldPart)
{
...
void KoaStack::FillTrackArray()
{
  ...
  fLogger->Debug(MESSAGE_ORIGIN, "KoaStack: Filling MCTrack array...");
  ...
  for (Int_t iPart=0; iPart<fNParticles; iPart++) {
    ...
    if (fStoreIter == fStoreMap.end() ) {
      fLogger->Fatal(MESSAGE_ORIGIN, "KoaStack: Particle %i not found in storage map! ", iPart);
    ...
    if (store) {
      KoaMCTrack* track =
        new( (*fTracks)[fNTracks]) KoaMCTrack(GetParticle(iPart));
      fIndexMap[iPart] = fNTracks;
      // --> Set the number of points in the detectors for this track
      for (Int_t iDet=kKoaRec; iDet<kSTOPHERE; iDet++) {
...
void KoaStack::UpdateTrackIndex(TRefArray* detList)
{
  ...
  fLogger->Debug(MESSAGE_ORIGIN, "KoaStack: Updating track indizes...");
  Int_t nColl = 0;

  // First update mother ID in MCTracks
  for (Int_t i=0; i<fNTracks; i++) {
    KoaMCTrack* track = (KoaMCTrack*)fTracks->At(i);
    Int_t iMotherOld = track->GetMotherId();
    fIndexIter = fIndexMap.find(iMotherOld);
    if (fIndexIter == fIndexMap.end()) {
      fLogger->Fatal(MESSAGE_ORIGIN, "KoaStack: Particle index %i not found in dex map! ", iMotherOld);
  ...
  while( (det = (FairDetector*)fDetIter->Next() ) ) {
    ...
    while ( (hitArray = det->GetCollection(iColl++)) ) {
      ...
      for (Int_t iPoint=0; iPoint<nPoints; iPoint++) {
        ...
        if (fIndexIter == fIndexMap.end()) {
          fLogger->Fatal(MESSAGE_ORIGIN, "KoaStack: Particle index %i not found in index map! ", iTrack);
          Fatal("KoaStack::UpdateTrackIndex", "Particle index not found in map");
        }
...
// -----   Public method Print  --------------------------------------------
void KoaStack::Print(Int_t iVerbose) const
{
  cout << "-I- KoaStack: Number of primaries        = "
       << fNPrimaries << endl;
  ...
  if (iVerbose) {
    for (Int_t iTrack=0; iTrack<fNTracks; iTrack++) {
      ((KoaMCTrack*) fTracks->At(iTrack))->Print(iTrack);
    }
  }
}
// -------------------------------------------------------------------------
...
void KoaStack::AddPoint(DetectorId detId)
{
  Int_t iDet = detId;
...
void KoaStack::AddPoint(DetectorId detId, Int_t iTrack)
{
  if ( iTrack < 0 ) { return; }
  Int_t iDet = detId;
...
Int_t KoaStack::GetCurrentParentTrackNumber() const
{
...
TParticle* KoaStack::GetParticle(Int_t trackID) const
{
  if (trackID < 0 || trackID >= fNParticles) {
    fLogger->Debug(MESSAGE_ORIGIN, "KoaStack: Particle index %i out of range.",trackID);
