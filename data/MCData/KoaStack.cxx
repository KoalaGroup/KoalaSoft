...
void KoaStack::FillTrackArray()
{
  ...
  for (Int_t iPart = 0; iPart < fNParticles; iPart++)
  {
    ...
    if (fStoreIter->second)
    {
      fIndexMap[iPart] = fNTracks;
      TParticle *part = GetParticle(iPart);
      KoaMCTrack *track =
          new ((*fTracks)[fNTracks]) KoaMCTrack(part);
      fNTracks++;
    }
    else
    {
...
void KoaStack::SelectTracks()
{
  ...
  for (Int_t i = 0; i < fNParticles; i++)
  {
    ...
    Int_t nPoints = 0;
    if (fPointsMap.end() != fPointsMap.find(i))
    {
      for (auto const &entry : fPointsMap.at(i))
      {
        nPoints += entry.second;
      }
    }
