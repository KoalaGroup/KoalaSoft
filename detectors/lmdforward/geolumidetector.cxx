...
geolumidetector::geolumidetector(const ptree& geometry_property_tree_) :
...
geolumidetector::~geolumidetector() {
}

void geolumidetector::init(FairGeoLoader* geoLoad) {
	retrieveMaterial(geoLoad);
}
void geolumidetector::retrieveMaterial(FairGeoLoader* geoLoad) {
	...
	if (!FairMediumAir || !FairMediumSteel || !FairMediumAl || !FairMediumSilicon
	    || !FairMediumVacuum) {
		LOG(ERROR) << "geolumidetector::retrieveMaterial: not all media found ";
		return;
	}
...
TGeoVolumeAssembly* geolumidetector::generateLmdGeometry() const {
	auto pt_general = geometry_property_tree.get_child("general");
...
TGeoRotation* lmd_ori = new TGeoRotation("lmd_ori");
	lmd_ori->RotateX(0.0);
	lmd_ori->RotateY(0.0);
	lmd_ori->RotateZ(0.0);
	...
	TGeoCombiTrans* lmd_frame_transformation = new TGeoCombiTrans("lmd_translation", 0.0,
								      0., box_center_position_z, lmd_ori); // x was  shift_x_from_dipole
TGeoCombiTrans* lmd_half_pos = new TGeoCombiTrans("lmd_half_pos", 0.0,
							  0.0 , 0.0, lmd_ori);
...
TGeoVolumeAssembly* top = new TGeoVolumeAssembly("lmd_top");
	...
	// generate beam pipe segment
		lmd_vol->AddNode(generateBeamPipe(), 0);           //wieder rein nehmen
	
	// add half detectors
	lmd_vol->AddNode(generateDetectorHalf(true), 0, lmd_half_pos); //wieder rein nehmen
	//lmd_vol->AddNode(generateDetectorHalf(false), 1);
...
TGeoVolume* geolumidetector::generateLmdBox() const {
	auto pt_general = geometry_property_tree.get_child("general");
...
TGeoVolume* geolumidetector::generateLmdBoxsecondary() const {
	auto pt_general = geometry_property_tree.get_child("general");
	auto pt_vac_box = geometry_property_tree.get_child("vacuum_box");
...
TGeoVolume* geolumidetector::generateBeamPipe() const {
	...
	lmd_vol_pipe_up->SetLineColor(kBlue);
...
TGeoVolume* geolumidetector::generateAluminumCoolingStructure() const {
...
TGeoVolume* geolumidetector::generatePCB() const {
...
TGeoVolume* geolumidetector::generatePCBMountScrew() const {
...
TGeoVolume* geolumidetector::generatePCBCopperPlug() const {
...
TGeoVolume* geolumidetector::generatePCBBacksideElectronics() const {
...
TGeoVolume* geolumidetector::generateSensorModule() const {
	...
	for (unsigned int i = 0; i < num_sensors_per_module_side; ++i) {
		...
		TGeoCombiTrans* rottrans_side_offset =
		    new TGeoCombiTrans(
				       (current_row_x_shift + i*0.005 + 0.5 * sensor_dim_x + cvd_disc_inner_radius
		            + sensor_dim_x * column_counter ), 0.5 * sensor_dim_y + current_row_y_shift, -0.5 * sensor_thickness - 0.5 * cvd_disc_thickness, rot_all_sensors_front);
...
TGeoVolume* geolumidetector::generateCVDCoolingDisc() const {
	auto pt_cool_support = geometry_property_tree.get_child("cooling_support");
...
TGeoVolume* geolumidetector::generateSensor() const {
	...
	double sensor_position_x(
	    geometry_property_tree.get<double>("general.sensor_position_x"));
	double sensor_position_y(
	    geometry_property_tree.get<double>("general.sensor_position_y"));
	...
	TGeoTranslation* trans_sensor_active = new TGeoTranslation("trans_sensor_active",
	    -sensor_dim_x / 2.0 + active_part_dim_x / 2.0 + active_part_offset_x + sensor_position_x,
	    +sensor_dim_y / 2.0 - active_part_dim_y / 2.0 + active_part_offset_y + sensor_position_y, 0.);
...
TGeoVolume* geolumidetector::generateDetectorHalf(bool is_bottom_half) const {
...
TGeoVolume* geolumidetector::generateDetectorHalfPlane(bool is_bottom_half) const {
	...
	lmd_vol_plane->AddNode(pcb_board_half_ring, 1, trans_pcb_half_ring_back); //wieder rein nehmen
...
TGeoVolume* geolumidetector::generateBoxVacuum(const TGeoVolume* lmd_vol) const {
	auto pt_vac_box = geometry_property_tree.get_child("vacuum_box");
	// general remark: watch out! TGeoShape values are often given has half sizes!
	double box_dim_x(pt_vac_box.get<double>("dimension_x"));
	double box_dim_y(pt_vac_box.get<double>("dimension_y"));
...
void geolumidetector::recursiveNodeSubtraction(std::stringstream& ss, TGeoNode* node) const {
...
void geolumidetector::makeNodesAlignable() const {
...
void geolumidetector::makeNodesAlignable(TGeoNode* node,
	...
	if (found) {
		if (std::string(node->GetName()).find(navigation_paths[current_navigation_path_index].first)
			...
			std::runtime_error("geolumidetector::makeNodesAlignable(): Volume path structure "
					"is mismatching!");
		}
	}
