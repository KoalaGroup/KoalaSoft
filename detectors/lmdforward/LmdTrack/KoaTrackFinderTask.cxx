...
#include "KoaTrackFinderTask.h"
#include "TStopwatch.h"
// #include "PndSdsPixelCluster.h"

// -----   Default constructor   -------------------------------------------
KoaTrackFinderTask::KoaTrackFinderTask(Int_t inFinderMode,
...
// -----   Destructor   ----------------------------------------------------
KoaTrackFinderTask::~KoaTrackFinderTask() {}
// -------------------------------------------------------------------------

// -----   Initialization  of Parameter Containers -------------------------
void KoaTrackFinderTask::SetParContainers() {}

InitStatus KoaTrackFinderTask::ReInit() {
  InitStatus stat = kERROR;
  return stat;
}

// -----   Public method Init   --------------------------------------------
InitStatus KoaTrackFinderTask::Init() {
  FairRootManager *ioman = FairRootManager::Instance();

  if (!ioman) {
    std::cout << "-E- KoaTrackFinderTask::Init: "
  ...
  if (!fPixelHitArray) {
    std::cout << "-W- KoaTrackFinderTask::Init: "
              << "No fPixelHitArray!" << std::endl;
    return kERROR;
  }

  fTrackCandArray = new TClonesArray("PndTrackCand");
  ioman->Register("LMDTrackCand", "Koa", fTrackCandArray, kTRUE);

  //  fTruePointArray = new TClonesArray("PndSdsMCPoint");

  std::cout << "-I- KoaTrackFinderTask: Initialisation successfull"
            << std::endl;
  return kSUCCESS;
}
// -------------------------------------------------------------------------
...
// --------------------------------------------
bool KoaTrackFinderTask::SortHitsByDet(
...
// --------------------------------------------
bool KoaTrackFinderTask::SortHitsByDet2(
    std::vector<std::vector<std::pair<Int_t, bool> > > &hitsd,
    Int_t nPixelHits) {
  Int_t nPlanes = 0;
...
// --------------------------------------------
bool KoaTrackFinderTask::SortHitsByZ(
    std::vector<std::vector<std::pair<Int_t, bool> > > &hitsd,
    Int_t nPixelHits) {
...
// --------------------------------------------
void KoaTrackFinderTask::FindHitsIII(
...
// --------------------------------------------
void KoaTrackFinderTask::FindHitsII(
    std::vector<PndTrackCand> &tofill,
    std::vector<std::vector<std::pair<Int_t, bool> > > &hitsd,
    Int_t)  // nPixelHits //FIXME [R.K.03/2017] unused variable(s)
{
...
void KoaTrackFinderTask::FindHitsI(
    std::vector<PndTrackCand> &tofill,
    std::vector<std::vector<std::pair<Int_t, bool> > > &hitsd,
    Int_t)  // nPixelHits //FIXME [R.K.03/2017] unused variable(s)
{
...
// -----   Public method Exec   --------------------------------------------
void KoaTrackFinderTask::Exec(Option_t *) {
...
Double_t KoaTrackFinderTask::GetTrackCurvature(KoaMCTrack *myTrack) {
  TVector3 p = myTrack->GetMomentum();
  return (2 / TMath::Sqrt(p.Px() * p.Px() + p.Py() * p.Py()));
}
// -------------------------------------------------------------------------

Double_t KoaTrackFinderTask::GetTrackDip(KoaMCTrack *myTrack) {
  TVector3 p = myTrack->GetMomentum();
  return (p.Mag() / TMath::Sqrt(p.Px() * p.Px() + p.Py() * p.Py()));
}

// -------------------------------------------------------------------------
ClassImp(KoaTrackFinderTask);
