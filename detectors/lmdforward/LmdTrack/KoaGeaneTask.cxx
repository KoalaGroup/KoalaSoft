...
// -----   Default constructor   -------------------------------------------
KoaGeaneTask::KoaGeaneTask()
    : FairTask("Geane Task for PANDA Lmd"), fEventNr(0), fUseMVDPoint(false) {
  track_branch_name = "KoaTrackFilt";
}
// -------------------------------------------------------------------------

KoaGeaneTask::KoaGeaneTask(Double_t pBeam, TVector3 IP,
                                 bool is_prefiltered)
    : FairTask("Geane Task for PANDA Lmd"), fEventNr(0), fUseMVDPoint(false) {
  fPDGid = -2212;
  fPbeam = pBeam;
...
// -----   Public method Init   --------------------------------------------
InitStatus KoaGeaneTask::Init() {
  // Get RootManager
  FairRootManager* ioman = FairRootManager::Instance();
  if (!ioman) {
    std::cout << "-E- KoaGeaneTask::Init: "
...
// -------------------------------------------------------------------------
void KoaGeaneTask::SetParContainers() {}

// -----   Public method Exec   --------------------------------------------
void KoaGeaneTask::Exec(Option_t*) {
  ...
  // fMCTracks->Delete();
  // fMCHits->Delete();
  if (fVerbose > 2) cout << "KoaGeaneTask::Exec END!" << endl;
}

void KoaGeaneTask::Finish() {
  //  tprop->Write();
}

FairTrackParP* KoaGeaneTask::PropToPlane(FairTrackParP* fStartPst,
                                            double zpos, int dir,
                                            bool& isProp) {
