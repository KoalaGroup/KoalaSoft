...
// -----   Default constructor   -------------------------------------------
KoaTrksFilterTask::KoaTrksFilterTask()
  ...
  flBOXKinFilt = false;
  flXThKinFilt = false;
  fHitName = "KOALAHitsMerged";
  fMCHitName = "KOALAPoint";
  //  fClusterName = clusterBranch;
  //  fDigiName = digiBrunch;
  fTrkCandName = "KOALATrackCand";
  fTrkName = "KOALAPndTrack";
  fTrkOutName = "KOALAPndTrackFilt";
}
...
// -----   Public method Init   --------------------------------------------
InitStatus KoaTrksFilterTask::Init() {
  // Get RootManager
  FairRootManager* ioman = FairRootManager::Instance();
  if (ioman == 0) {
    Error("KoaTrksFilterTask::Init", "RootManager not instantiated!");
    return kERROR;
  }

  fTrkArray = (TClonesArray*)ioman->GetObject(fTrkName);
  if (fTrkArray == 0) {
    Error("KoaTrksFilterTask::Init", "track-array not found!");
    return kERROR;
  }

  fTrkCandArray = (TClonesArray*)ioman->GetObject(fTrkCandName);
  if (fTrkCandArray == 0) {
    Error("KoaTrksFilterTask::Init", "trk-cand--array not found!");
    return kERROR;
  }

  fTrkOutArray = new TClonesArray("PndTrack");
  ioman->Register("KOALAPndTrackFilt", "Koa", fTrkOutArray, kTRUE);
...
// -----   Public method Exec   --------------------------------------------
void KoaTrksFilterTask::Exec(Option_t*) {
  fTrkOutArray->Delete();
  if (fVerbose > 4) {
    cout << "" << endl;
  ...
  if (fVerbose > 2) cout << "KoaTrksFilterTask::Exec END!" << endl;
  fEventNr++;
}

void KoaTrksFilterTask::FinishTask() {}

ClassImp(KoaTrksFilterTask);
