int LmdEff(const int nEvents = 10, const int startEvent = 0,
	...
	TString TCandFile = storePath + "/Koala_Track_"; //"/Lumi_digi_noise_";//"/Lumi_digi_";
	TCandFile += startEvent;
	TCandFile += ".root";
	...
	TString BackTrackFile = storePath + "/Koala_TrackEff_"; //"/Lumi_digi_noise_";//"/Lumi_digi_";
	BackTrackFile += startEvent;
	BackTrackFile += ".root";
	...
	input_source.AddFriend(TCandFile);
	input_source.AddFriend(BackFile);
	input_source.AddFriend(CompFile);
	input_source.AddFriend(BackTrackFile);
...
// fRun->LoadGeometry();
	...
	// =========================================================================
	// -----    LMD hit producer   --------------------------------------------
	LmdEffi* lmdEff = new LmdEffi();
	lmdEff->SetVerbose(verboseLevel);
	fRun->AddTask(lmdEff);
