int KoaPixel6Compare(const int nEvents = 10, const int startEvent = 0,
		TString storePath = "tmpOutput", const int verboseLevel = 2, bool misalign =
				false) {
	// ========================================================================
	...
	// Output file
	TString outFile = storePath + "/Koala_comp_";
	outFile += startEvent;
	outFile += ".root";
...
// fRun->LoadGeometry();
	...
	// =========================================================================
	// -----    LMD hit producer   --------------------------------------------
	KoaComp* koacom = new KoaComp();
	koacom->SetVerbose(verboseLevel);
	fRun->AddTask(koacom);
