                             ...
                             )
{
  ...
  // histograms
  TString h1name("cluster_multiplicity");
  TString h1title("Cluster Multiplicity");
  TString hDirName = "cluster_multiplicity";
  ...
  if(useClusterSize) {
    h1name.Append("_ClusterSize");
    h1title.Prepend("(Cluster Size Limit)");
    hDirName.Append("_ClusterSize");
  }
  ...
  //                                  );
  //
  auto h1map = new TH1D(h1name.Data(),
                        h1title.Data(),
                        10, -0.5, 9.5);
  ...
  for(auto entry=0;entry<entries;entry++){
    ...
    for (int i=0;i<clusters;i++){
      ...
      if (useClusterSize && nr_digis > size_limit[det_id]) {
        continue;
      }
  ...
  hdir->WriteTObject(h1map, "", "Overwrite");
