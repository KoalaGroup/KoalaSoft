#include "checkClusterVsTof.C"

void drawClusterVsTof_batch(const char* indir,
                            const char* suffix,
                            const char* geoFile
                            )
{
  ...
  for (int i=0;i<16;i++) {
    ...
    fwdhit_file = infile;
    fwdhit_file.ReplaceAll(".root", "_FwdHit.root");
    checkClusterVsTof(infile.Data(), "koalasim", "KoaRecCluster",
                      fwdhit_file.Data(), "fwdhit_time",
                      500, 0, 10, 1500, 450, 750,
                      geoFile
                      );
  }
