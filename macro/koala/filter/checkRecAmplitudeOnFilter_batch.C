#include "checkRecAmplitudeOnFilter.C"

void checkRecAmplitudeOnFilter_batch(const char* indir)
{
  TString filelist [] ={
  ...
  TString elistfile;
  for (int i=0;i<16;i++) {
    infile = indir + filelist[i] + "_calib.root";
    elistfile = indir + filelist[i] + "_EntryList.root";
    checkRecAmplitudeOnFilter(infile.Data(), "koalasim",
                              elistfile.Data()
                              );
  }
