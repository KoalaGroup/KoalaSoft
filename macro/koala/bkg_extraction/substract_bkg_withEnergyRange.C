...
void substract_bkg_withEnergyRange(
                                   const char* filename,
                                   int ref_ch = 3,
                                   ...
                                   )
{
  ...
  double ref_count = get_integral(h1s[ref_id]);
  ...
  auto dir_out = getDirectory(fout, Form("no_bkg_withEnergyRange_low%.2f_high%.2f", e_low, e_high));
  writeHistos<TH1D>(dir_out, hsub);
  dir_out = getDirectory(fout, Form("divide_bkg_withEnergyRange_low%.2f_high%.2f", e_low, e_high));
  writeHistos<TH1D>(dir_out, hdiv);

  TString pdffilename(filename);
  pdffilename.ReplaceAll(".root", Form("_NoBkgWithEnergyRange_low%.2f_high%.2f.pdf", e_low, e_high));
