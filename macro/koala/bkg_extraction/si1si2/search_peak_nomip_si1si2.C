...
void search_peak_nomip_si1si2(
                              const char* filename,
                              const char* dirname = "no_mip_Si1Si2",
                              const char* suffix = "nomip"
                              )
{
  // get ref ch id
  auto encoder = KoaMapEncoder::Instance();
  auto si1_id_start = encoder->EncodeChannelID(0,27);
  auto si1_id_stop = encoder->EncodeChannelID(0,47);
  auto si2_id_start = encoder->EncodeChannelID(1,1);
  ...
  for(auto item: h1s){
    ...
    if(id < si1_id_start) continue;
    else if (id > si1_id_stop && id < si2_id_start) continue;
    ...
    if(np == 1){
      ...
      auto result = hist->Fit("gaus", "qs", "", xpeaks[0]-0.1, xpeaks[0]+0.1);
  ...
  pdffilename.ReplaceAll(".root", "_FitElasticPeak_Si1Si2.pdf");
  printHistos<TH1D*>(h1s, pdffilename.Data(), "", true);

  TString outfile_txt(filename);
  outfile_txt.ReplaceAll(".root","_FitElasticPeak_Si1Si2.txt");
