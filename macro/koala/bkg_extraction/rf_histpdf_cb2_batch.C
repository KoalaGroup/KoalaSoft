...

{
  ...
  auto fit_histpdf_cb2 = [&] ()
                         {
                           ...
                           outfile_pdf.ReplaceAll(".root", Form("_%s_fitted_HistPdf_CB2Shape.pdf", hdir_name.Data()));

                           TCanvas *can = new TCanvas("canvas","Fitting using HistPdf+CB2Shape", 1500, 500);
                           can->Divide(3,1);
  ...
  outdir_name.Append("_fitted_HistPdf_CB2Shape");
  ...
  outfile_channel_txt.ReplaceAll(".root", Form("_%s_fitted_HistPdf_CB2Shape_Channels.txt", hdir_name.Data()));
