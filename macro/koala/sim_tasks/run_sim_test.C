void run_sim_test(Double_t beamMom = 2.6, Int_t nEvents = 1000, const char* outdir="./", TString mcEngine = "TGeant4")
{
  // ----    Debug option   -------------------------------------------------
  gDebug = 0;
  ...
  // Output file name
  TString outFile =Form("%s/test.root", outdir);
    
  // output Parameter file name
  TString parFile=Form("%s/test_param.root", outdir);
