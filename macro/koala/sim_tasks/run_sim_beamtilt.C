// Run simulation with beam tilt against y-axis
// beam_tilt unit is degree
void run_sim_beamtilt(Double_t beamMom = 2.6, Int_t nEvents = 100,
                        double beam_tilt = 0., 
                        const char* outdir="./", TString mcEngine = "TGeant4")
{
  // ----    Debug option   -------------------------------------------------
  gDebug = 0;
  ...
  primGen->SetBeam(0, 0, 1., 1.); // beam offset: 4 mm, beam width: 10 mm

  // Tilt beam direction
  primGen->SetBeamAngle(0, beam_tilt/180*TMath::Pi(), 0, TMath::Pi()/180); // 1 degree sigma
