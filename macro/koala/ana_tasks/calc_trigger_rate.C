...
void calc_trigger_rate(const char* infile,
                       const char* outfile,
                       int half_window = 2)
{
  ...
  auto tin = (TTree*)fin->Get("TriggerSumCount");
  Long64_t sec, usec;
  ...
  auto tout = new TTree("KOALA_Trigger_Rates", "Rates from KOALA Trigger");
  ...
  tout->Branch("KOALA_Trigger_Rate", &rates);
